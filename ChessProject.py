...
class Board(object):
    ...
    def check_mate(self):
        ...
        self.whiteKing = (self.__board[white_coordinates[0]][white_coordinates[1]]).legal_moves
        self.blackKing = (self.__board[black_coordinates[0]][black_coordinates[1]]).legal_moves

        if (len(self.whiteKing) == 0 and self.WK_Check == True) != False:
            print("Black Wins")
            return ("B",white_coordinates)

        if (len(self.blackKing) == 0 and self.BK_Check == True) != False:

            print("White Wins")
            return ("W",black_coordinates)
